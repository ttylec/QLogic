...
-- |
-- = General partially ordered sets

-- | Class for types representing partially ordered sets.
-- Method @supIn@ is defined so that 
-- specific instances can provide more efficient
-- implementation.
...
-- | General partially ordered set type. 
data Poset a where
...
isPoset poset = isPoset' . fromPOrdStruct $ poset
