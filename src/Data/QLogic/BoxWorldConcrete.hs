...
concreteLogic :: IntSet -> [IntSet] -> QLogic ConcretePosetInt IntSet
concreteLogic space els = fromPoset (ConcretePosetInt els) booleanOcmpl 
    where
        booleanOcmpl = difference space

boxWorldCLogic2 :: [Observable] -> [Observable] -> (Question -> IntSet, QLogic ConcretePosetInt IntSet)
