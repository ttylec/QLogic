...
instance (Ord a, Show a) => Show (Concrete a) where
  show poset = "No. of elements: " ++ (show . length $ elementsOf poset) ++
               "\nGreater than lists:\n" ++ unlines gtlists
    where
      gtlists = map (\a -> show a ++ "| " ++ show (geEqThan poset a))
                $ elementsOf poset
...
-- | Generate concrete sub-OML of `ConcreteInt`
concreteIntSubOML :: ConcreteInt -> [IntSet] -> ConcreteInt
concreteIntSubOML ql =  ConcreteInt (oneOf ql) . generateOML ql

concreteIntSubOMP :: ConcreteInt -> [IntSet] -> ConcreteInt
concreteIntSubOMP ql = ConcreteInt (oneOf ql) . generateOMP ql
...
concreteSubOMP ql = Concrete (oneOf ql) . generateOMP ql

concreteSubEA :: Ord a => Concrete a -> [Set a] -> Concrete a
concreteSubEA ql = Concrete (oneOf ql) . generateEA ql
